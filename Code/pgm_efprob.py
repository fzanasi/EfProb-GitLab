...
def inference_map_query(stretch_dict, variables, evidence_dict={}):
    if len(variables) == 0:
...
def inference_map_query1(stretch_dict, variables):
    ...
    for i in range(chan_list_len - 1, -1, -1):
        if var_list[i][0] == 1:
            top_var_index = i
            break
    print("Top var index", top_var_index)
    chan = chan_list[top_var_index]
    maps = (chan % var_list[top_var_index][1]).MAP()
    chan = chan_list[top_var_index - 1]
    if var_list[top_var_index - 1][0] == 1:
        last_mask = var_list[top_var_index - 1][1]
    else:
        last_mask = len(chan.cod) * [0]
    print( chan_list_len, last_mask )
    print( "\ninitial occ map on:", chan_list[top_var_index].dom.names )
    print( "initial channel from:", chan.dom.names, chan.cod.names )
    for i in range(top_var_index - 1, -1, -1):
        if i == 0:
            print("break")
            break
        if var_list[i-1][0] == 0:
    ...
    while len(unvisited_nodes) > 0:
        for n in unvisited_nodes: 
            if any([c in unvisited_nodes for c in children[n]]):
                continue
            offspring[n] = [n] + children[n] + [m for c in children[n] 
                                                for m in offspring[c] ]
            unvisited_nodes.remove(n)
    # remove duplicates
    for n in nodes:
        offspring[n] = set(offspring[n])
    observed_nodes = [marginal] + list(evidence.keys())
    nodes_copy = [n for n in nodes]
    for n in nodes_copy:
        if any([(on in offspring[n]) for on in observed_nodes]):
            continue
        # print("Remove", n, offspring[n] )
        pgm.remove_node(n)
    if not silent:
        print("Observed:", observed_nodes)
        graph = pydot_graph_of_pgm(pgm)
        graph_image(graph, "test")
    stretched_pgm = stretch(pgm, silent=silent)
...
# version with ordering of nodes, given by fraction of (sizes of)
# parent and children nodes; this does not lead to a narrower tretch,
# probably because the width gets pushed to the end.

def stretch(pgm, graph_output=False, observed=False, stretch_tries=10000):
    #
    # Extraction of relevant data from input
    #
    nodes = pgm.nodes
    ...
    for n in nodes:
        if len(parents[n]) == 0:
            ...
            parents[n] = [dn.name for dn in channels[n].dom.names]
    #
    # The fraction between the sizes of the parent and children nodes
    # is recorded and used to order the non-initial nodes.
    node_scores = {}
    for n in non_initial_nodes:
        node_scores[n] = list_size(children[n]) / list_size(parents[n])
    non_initial_nodes.sort(key = lambda n: node_scores[n])
    print( node_scores )
    print( non_initial_nodes )
    #
    # Initialisation of data structures that will be build up: (a)
    # list of channels, (b) nodes pointers, (c) node copies, (d)
    # graph, optionally
    ...
    for st in range(stretch_tries):
        ...
        for n in initial_nodes:
            node_copies[n] = len(children[n]) + (1 if observed else 0)
        unprocessed_nodes = non_initial_nodes
        iterations = 0
        while len(unprocessed_nodes) > 0:
            for un in unprocessed_nodes:
                iterations += 1
