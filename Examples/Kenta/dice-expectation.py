...
rv = randvar_fromfun(lambda x: x, pips)
...
rv_sum2 = randvar_fromfun(lambda x, y: x + y, [pips, pips])
...
def rv_sum(n):
    return randvar_fromfun(lambda *xs: sum(xs), [pips] * n)

for n in [2, 3, 5]:
    print("n = {}:".format(n), rv_sum(n).exp(s_dice ** n))

def rv_even_game(n):
    return randvar_fromfun(lambda *xs:
...
rv_prime_game = randvar_fromfun(lambda x: x if x in [2, 3, 5] else -x,
                                [pips])

print(rv_prime_game.exp(s_dice))

rv_umbrella_sales = randvar_fromfun(lambda x: 300 if x else -60,
                                    [True, False])
s_rain = flip(0.3)
